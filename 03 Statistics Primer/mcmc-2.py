#!/usr/bin/env python
# MCMC sampler using Metropolis-Hastings algorithm in 2D
...
def step(x):
	...
	reject = uniform(size=[1,chains]) > alpha
	#print('acceptance rate = %.3f' % (1.0 - reject.sum()/chains))
	return np.where(reject, x, y)
