...
! compile with: gfortran -O3 -fdefault-real-8 -fopenmp mandelbrot-parallel.f90 -lcfitsio [-lcurl]
...
do j = 1,ny
do i = 1,nx
                data(1,i,j) = escape(xx(1) + (xx(2)-xx(1))*(i-1)/(nx-1), yy(1) + (yy(2)-yy(1))*(j-1)/(ny-1))
end do
end do
...
pure function escape(x, y); intent(in) x,y
        real x, y, escape
        complex z, c
        integer k
        ...
        if (k < iterations) escape = k+1 - log(log(abs(z))/log(2.0))/log(2.0)
end function
