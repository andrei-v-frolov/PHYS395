! mandelbrot.f90 - 2D image output demo (OpenMP parallel version)
! gfortran -O3 -mandelbrot-parallel.f90 -fopenmp -lcfitsio

program mandelbrot; implicit none

integer, parameter :: nx = 3 * 2**10, ny = 2**11, iterations = 1000
...
!$omp parallel do
do i = 1,nx; do j = 1,ny
	image(1,i,j) = mandel_iter(&
                cmplx(xrange(1) + (xrange(2) - xrange(1))*(i-1)/(nx-1),&
                        yrange(1) + (yrange(2) - yrange(1))*(j-1)/(ny-1)))
end do; end do
...
elemental function mandel_iter(c)
        complex c, z
        integer k, mandel_iter
        intent(in) c
